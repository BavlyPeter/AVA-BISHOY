# اريبصالين (avabishoy) - Church Festival & Sunday School Management System
...
1. **Morning Arrival Bottlenecks & Queuing Delays:**  
   Handling hundreds of children arriving simultaneously during Sunday liturgy or summer school previously resulted in paper-roster bottlenecks. avabishoy's hardware-accelerated QR scanner decodes badges and updates reactive state in under 300ms, providing instant visual feedback and background database persistence.
...
3. **Loss and Wear of Physical Badges:**  
   Physical paper badges degrade or get lost. avabishoy renders pixel-perfect, Coptic-themed Smart ID Badges (`IDCard.tsx`) entirely in the browser using SVG and canvas rendering, exportable as high-resolution PNGs or batch-downloadable PDF card decks.
...
The system enforces 5 distinct roles governed by [`RoleGuard.tsx`](file:///D:/avabishoy/avabishoy/src/components/auth/RoleGuard.tsx):
...
Located in [`TeachersPage.tsx`](file:///D:/avabishoy/avabishoy/src/pages/TeachersPage.tsx):
...
Located in [`LoginPage.tsx`](file:///D:/avabishoy/avabishoy/src/pages/LoginPage.tsx):
...
4. **Credential Synthesis:** Combines the resolved `teacher_id` into a standardized synthetic email: `${teacherId.toLowerCase()}@avabishoy.com`.
5. Authenticates against Supabase Auth: `supabase.auth.signInWithPassword({ email, password })`.
6. [`AuthInitializer.tsx`](file:///D:/avabishoy/avabishoy/src/components/auth/AuthInitializer.tsx) catches the active session, loads the servant's database record, and hydrates the global Zustand store.

---

#### Workflow D: Compensating Point Rollbacks & Overdraft Guards
Located in [`MarketModal.tsx`](file:///D:/avabishoy/avabishoy/src/components/modals/MarketModal.tsx) & [`ManualPointsModal.tsx`](file:///D:/avabishoy/avabishoy/src/components/modals/ManualPointsModal.tsx):
...
Defined in [`useFestivalStore.ts`](file:///D:/avabishoy/avabishoy/src/store/useFestivalStore.ts):
...
│       └── service logo.png             # Official avabishoy Ministry emblem
├── components/
│   ├── auth/
...
The visual identity reflects the liturgical heritage of the Coptic Orthodox Church, defined in [`theme.css`](file:///D:/avabishoy/avabishoy/src/styles/theme.css):
